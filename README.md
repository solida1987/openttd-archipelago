...
When a seed was generated against a set you do not have, the launcher offers
to fetch it — and that download comes from **BaNaNaS**, OpenTTD's own content
service, using the content client the game already ships with. The file goes
from its authors to you; nothing passes through us, and only the sets the seed
actually names are fetched. Every package on that service is published under
one of OpenTTD's declared licences (GPL-2.0/3.0, CC-0, CC BY, CC BY-SA,
CC BY-NC-SA or CC BY-NC-ND). You can always decline and install them yourself
through the game's own *Check Online Content*.
