...
def main():
    ...
    src = os.path.join(ROOT, "apworld", "openttd_exp.apworld")
    if not os.path.isfile(src):
        print("no apworld/openttd_exp.apworld -- run tools/build_apworld.py")
        return 1
    ...
    try:
        ...
        os.makedirs(players)
        os.makedirs(output)
        # The generated schema names the STABLE game; the apworld installed here
        # is the experimental line, which calls itself OpenTTD-Exp.
        n = write_yaml(os.path.join(players, "gate.yaml"), game + "-Exp", rows)
