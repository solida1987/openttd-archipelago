...
:: ============================================================
::  OpenTTD Archipelago — STABLE Build + Package
::  Builds from the EXP working directory with stable branding.
::  Patches "OpenTTD-Exp" → "OpenTTD" before build, reverts after.
::
...
:: ── STIER ───────────────────────────────────────────────────
:: Vi bygger direkte fra EXP-mappen — ingen separat kopi nødvendig
...
:: ── STEP 1: Patch game name + version tag til stable ────────
:: VIGTIGT: Vi gemmer backup af filerne FØR patching, og gendanner dem efter.
:: Dette sikrer at ucommittede ændringer IKKE går tabt.
echo [1/7] Patcher game name og version tag ...
...
:: Sæt midlertidigt git tag så CMake bruger "v1.2.0" som version
cd /d "%PROJECT_DIR%"
git tag -d v%AP_VERSION% > nul 2>&1
git tag v%AP_VERSION%
echo       OK: git tag = v%AP_VERSION% (CMake version string)

:: ── STEP 2: Visual Studio ──────────────────────────────────
echo [2/7] Aktiverer Visual Studio build-miljoe...
call "%VCVARS%" > nul 2>&1
echo       OK: %VCVARS%

:: ── STEP 3: vcpkg ──────────────────────────────────────────
echo [3/7] Tjekker vcpkg...
...
:: ── STEP 4: Clean build ────────────────────────────────────
echo [4/7] Forbereder build-mappe...
...
:: ── STEP 6: Build ──────────────────────────────────────────
echo.
echo [6/7] Bygger (RelWithDebInfo)...
echo ============================================================
...
:: ── Archipelago APWorld (patch openttd_exp → openttd for stable) ──
if exist "%PROJECT_DIR%\apworld" (
    echo   Patcher APWorld: openttd_exp -^> openttd ...
    :: Kopier source-mappen som openttd (ikke openttd_exp)
    mkdir "%OUT%\apworld"
    mkdir "%OUT%\apworld\openttd"
    xcopy /E /I /Q "%PROJECT_DIR%\apworld\openttd_exp" "%OUT%\apworld\openttd" > nul
    :: Patch game name i archipelago.json OG __init__.py
    powershell -NoProfile -Command ^
        "(Get-Content '%OUT%\apworld\openttd\archipelago.json') -replace 'OpenTTD-Exp', 'OpenTTD' | Set-Content '%OUT%\apworld\openttd\archipelago.json'"
    powershell -NoProfile -Command ^
        "(Get-Content '%OUT%\apworld\openttd\__init__.py') -replace 'OpenTTD-Exp', 'OpenTTD' | Set-Content '%OUT%\apworld\openttd\__init__.py'"
    :: Byg openttd.apworld ZIP (med openttd/ som rodmappe inde i ZIP)
    :: NB: Compress-Archive understøtter KUN .zip - så vi laver .zip først og omdøber til .apworld
    if exist "%OUT%\apworld\openttd.apworld" del /f "%OUT%\apworld\openttd.apworld"
    if exist "%OUT%\apworld\openttd.zip" del /f "%OUT%\apworld\openttd.zip"
    powershell -NoProfile -Command ^
        "Compress-Archive -Path '%OUT%\apworld\openttd' -DestinationPath '%OUT%\apworld\openttd.zip' -CompressionLevel Optimal"
...
:: ── Fiks resolution i openttd.cfg ──────────────────────────
if exist "%OUT%\data\openttd.cfg" (
    echo   Fixer resolution i openttd.cfg  800x600 ...
    ...
    echo   [OK]      data\openttd.cfg resolution sat til 800,600
)
...
if exist "%OUT%\apworld\openttd.apworld" (
    ...
    echo   [ADVARSEL] apworld\openttd.apworld mangler!
)
if exist "%OUT%\apworld\openttd\archipelago.json" (
    echo   [OK]      apworld\openttd\          (stable source)
) else (
    echo   [ADVARSEL] apworld\openttd\ source mangler!
)
...
:: ── Revert game name + tag tilbage til experimental ──────────
:: Gendanner backup-filer (bevarer alle ucommittede ændringer).
echo.
echo Reverter source-filer fra backup og fjerner git tag ...
cd /d "%PROJECT_DIR%"
...
git tag -d v%AP_VERSION% > nul 2>&1
echo       OK: git tag v%AP_VERSION% fjernet
...
echo.
echo  Naeste trin: Koer stable_release.bat for at pushe
echo  til GitHub og oprette release.
echo ============================================================
echo.
pause
exit /b 0

:: ── Fejl-handler: revert game name og afslut ────────────────
:revert_and_exit
echo.
...
git tag -d v%AP_VERSION% > nul 2>&1
echo       OK: game name og tag revertet til OpenTTD-Exp
pause
exit /b 1
