...
struct ArchipelagoMissionsWindow : public Window {
	...
	void OnRealtimeTick([[maybe_unused]] uint delta_ms) override {
		 ...
		 * without waiting for _ap_status_dirty (only set on mission completion). */
		if (_ap_status_dirty.exchange(false) || show_tasks) RebuildVisibleList();
		else this->SetDirty();
	}

	void OnInvalidateData([[maybe_unused]] int data = 0, [[maybe_unused]] bool gui_scope = true) override {
		/* Called by InvalidateWindowClassesData(WC_ARCHIPELAGO) every 250 ms
		 * from the AP realtime timer.  Always refresh so the tasks tab shows
		 * live progress without requiring user interaction. */
		if (!gui_scope) return;
		RebuildVisibleList();
	}
